  ...
  <nav class="navbar">
    <div class="navbar-brand is-size-4">
      <nuxt-link to="/" class="navbar-item navbar-logo">
        <img style="max-height: 5.75rem;" src="logo.png">
      </nuxt-link>
    ...
    <div class="navbar-menu" :class="{'is-active': showMenu}">
      ...
      <form class="navbar-end" @submit.prevent="search">
        <div class="navbar-item input-item">
          ...
          <button type="submit" class="button is-ccs" :class="{'is-loading': searching}">
            <Icon icon="search" />
          </button>
        </div>
      </form>
...
<script>
  import {get as ccsinfoGet} from '@/services/ccsinfo-api'
  ...
  export default {
    ...
    methods: {
      async search() {
        ...
        this.searching = true
        try {
          let {type, id, address} = await ccsinfoGet(`/search`, {params: {query: searchString}})
          switch (type) {
          case 'address':
            this.searchString = ''
